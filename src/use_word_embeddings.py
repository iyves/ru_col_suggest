...
USE_FASTTEXT = False
PREDICT = True
PREDICT_COMBINED = False
USE_COSSIM = True
USE_COSMUL = False
ATTEST_COLLOCATIONS = True
WRITE_COLLOCATIONS = False
...
def generate_word_pairs(sentences: Iterable[str]):
    ...
    for sentence in sentences:
        word_pairs.append([word.lower() for word in kutuzov.process(kutuzov.process_pipeline, text=sentence,
...
def get_attested_collocations_new(result):
    tokens = result[-3]
    words = result[-2]
    word_lemma = " ".join([n.split("_")[0] for n in words])
    pos_filtered_lists = result[-1]
    attested_collocations = WordEmbedder.filter_results(
        [[n[0] for n in list]
         for list in pos_filtered_lists])
...
def predict(model_type, model_path, word_pairs, token_pairs, predict_type="regular",
            cos_type="cossim", topn=100,
    ...
    results = []
    if predict_type == "regular":
        results = embedder.predict_similar(word_pairs=word_pairs, topn=topn, token_pairs=token_pairs,
                                           verbose=verbose, cos_type=cos_type)
    elif predict_type == "combined":
        # like list[list[tokens, lemma_pos_words, candidate_collocates]]
        # tokens should be optional: pass in opt arg to attest funct?
        results = embedder.predict_similar_combined(word_pairs=word_pairs, topn=topn, token_pairs=token_pairs,
                                                verbose=verbose, cos_type=cos_type)
    else:
        logging.error("Error: Invalid predict_type not 'regular' or 'combined': {}".format(predict_type))
        return

    if attest_collocations:
        for idx, result in enumerate(results):
            if predict_type == "regular":
                # already POS filtered
                # result like list[list[tokens, lemma_pos_words, candidates]]
                attested_collocations = result
            elif predict_type == "combined":
                # should filter POS here
            ...
            if write_collocations:
                if predict_type == "regular":
                    if include_header:
                        attested_collocations.insert(0, ["num_replacements", "token", "lemma", "suggested_collocation",
                                                         "sum_of_rank", "log_rank", "ngram_freq", "doc_freq",
                                                         "pmi", "t_score"])
                    print ("aaaah\n", attested_collocations)
                    write_result(attested_collocations, out_path + str(token_pairs[idx]) + ".txt")
                else:
                    if include_header:
...
def main():
    ...
    token_pairs = [
        ...
        # 'по убеждение автор'
        # 'в больший мера'
        # 'развитие общенародный хозяйство'
        #'сыграть важный роль'
        'социальный развитие страна'
    ]
        ...
        if USE_COSSIM:
            ...
            if PREDICT:
                predict(model_type="w2v", model_path=w2v_model, word_pairs=word_pairs, token_pairs=token_pairs,
                    predict_type="regular", cos_type="cossim", topn=100, verbose=not ATTEST_COLLOCATIONS,
                    attest_collocations=ATTEST_COLLOCATIONS, include_header=True,
                    write_collocations=WRITE_COLLOCATIONS, out_path=out_path)
            if PREDICT_COMBINED:
                predict(model_type="w2v", model_path=w2v_model, word_pairs=word_pairs, token_pairs=token_pairs,
                        predict_type="combined", cos_type="cossim", topn=100, verbose=not ATTEST_COLLOCATIONS,
                        attest_collocations=ATTEST_COLLOCATIONS, include_header=True,
                        write_collocations=WRITE_COLLOCATIONS, out_path=out_path)
        if USE_COSMUL:
            ...
            if PREDICT:
                predict(model_type="w2v", model_path=w2v_model, word_pairs=word_pairs, token_pairs=token_pairs,
                        predict_type="regular", cos_type="cosmul", topn=100, verbose=not ATTEST_COLLOCATIONS,
                        attest_collocations=ATTEST_COLLOCATIONS, include_header=True,
                        write_collocations=WRITE_COLLOCATIONS, out_path=out_path)
            if PREDICT_COMBINED:
                predict(model_type="w2v", model_path=w2v_model, word_pairs=word_pairs, token_pairs=token_pairs,
                        predict_type="combined", cos_type="cosmul", topn=100, verbose=not ATTEST_COLLOCATIONS,
                        attest_collocations=ATTEST_COLLOCATIONS, include_header=True,
                        write_collocations=WRITE_COLLOCATIONS, out_path=out_path)
    ...
    if USE_FASTTEXT:
        ...
        if USE_COSMUL:
            ...
            if PREDICT:
                predict(model_type="fastText", model_path=fastText_model, word_pairs=word_pairs, token_pairs=token_pairs,
                        cos_type="regular", topn=100, verbose=not ATTEST_COLLOCATIONS,
                        attest_collocations=ATTEST_COLLOCATIONS, include_header=True,
                        write_collocations=WRITE_COLLOCATIONS, out_path=out_path)
            if PREDICT_COMBINED:
                predict(model_type="fastText", model_path=fastText_model, word_pairs=word_pairs, token_pairs=token_pairs,
                        cos_type="combined", topn=100, verbose=not ATTEST_COLLOCATIONS,
                        attest_collocations=ATTEST_COLLOCATIONS, include_header=True,
                        write_collocations=WRITE_COLLOCATIONS, out_path=out_path)
